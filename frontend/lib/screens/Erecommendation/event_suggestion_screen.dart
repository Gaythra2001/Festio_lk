...
String formatDate(DateTime? date) {
 ...
 Future<void> loadSuggestions() async {
  ...
  clickData = (clicks ?? {})
      .map((k, v) => MapEntry(k.toString().trim(), v));
...
bool districtMatch = normalize(event['location'])
  ...
  debugPrint(
      "✅ Same District Events Found: ${sameDistrictEvents.length}");

  // Sort by duration first, then clicks
  for (var event in sameDistrictEvents) {
    final duration =
        await FirebaseService()
            .getEventTotalDurationByTitle(
                event['title']);

    event['duration'] = duration;

    event['clickCount'] =
        clickData?[event['title']
                .toString()
                .trim()] ??
            0;
...
bool differentDistrict =
    ...
    debugPrint(
        "⚠️ No Same District Events Found.");
  }
            ...
            if (similarOtherDistrictEvents.isNotEmpty) ...[
              ...
              SizedBox(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    final event =
                        similarOtherDistrictEvents[index];
                    return buildEventCard(event,
                        isMostPopular: false);
                  },
                ),
  ...
  MaterialPageRoute(
    builder: (_) => ModernEventDetailScreen(
      ...
      await FirebaseService().endEventSession(
          eventTitle: event['title'], openedAt: openedAt);

      // Increment clicks
      await FirebaseService()
          .incrementEventClick(event['title'].toString());
